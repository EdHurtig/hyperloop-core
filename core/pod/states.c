...
pod_t _pod = {
    ...
    .last_ping = 0,
    .manual_mode = false,
    .brake_timeout = BRAKING_TIMER,
    .emergency_timeout = EMERGENCY_TIMER,
...
int init_pod(void) {
  ...
  pod->hp_pressure = (sensor_t){
    ...
    .cal_d = HP_TRANSDUCER_CALIBRATION_D,
    .alpha = 1.0,
    .offset = 0.0,
    .mux = PRESSURE_MUX,
  ...
  for (i = 0; i < N_REG_PRESSURE; i++) {
    ...
    pod->reg_pressure[i] = (sensor_t){
      ...
      .cal_d = LP_TRANSDUCER_CALIBRATION_D,
      .alpha = 1.0,
      .offset = 0.0,
      .mux = PRESSURE_MUX,
  ...
  for (i = 0; i < N_REG_THERMO; i++) {
    ...
    pod->reg_thermo[i] = (sensor_t){
      ...
      .cal_d = FLOW_THERMO_CALIBRATION_D,
      .alpha = 0.1,
      .offset = 0.0,
      .mux = REG_THERMO_MUX,
      .input = reg_thermo[i]
  ...
  for (i = 0; i < N_REG_SURF_THERMO; i++) {
    ...
    pod->reg_surf_thermo[i] = (sensor_t){
      ...
      .cal_d = WHITE_THERMO_CALIBRATION_D,
      .alpha = 0.1,
      .offset = 0.0,
...
float update_sensor(sensor_t *sensor, int32_t new_value) {
  ...
  float calibrated =
      (sensor->cal_b * x * x) + (sensor->cal_c * x) + sensor->cal_d;
  float filtered =
